...
class Llama {
  ...
  void _initializeLlama(
      ...
      SamplerParams? samplerParams) {
    // Всегда показываем логи C для отладки
    ...
    if (!_backendInitialized) {
      ...
      if (Platform.isAndroid) {
        // Load CPU backend via ggml_backend_load so it's properly registered
        // in libggml.so's global registry table. Android dlopen finds
        // libggml-cpu.so in the app's native library dir (already loaded as
        // a dependency of libmtmd.so).
        lib.llama_backend_init();
        final cpuLibPath = 'libggml-cpu.so'.toNativeUtf8().cast<Char>();
        final reg = lib.ggml_backend_load(cpuLibPath);
        malloc.free(cpuLibPath);
        print("🟡 cpu_reg loaded: ${reg != nullptr}");
        print("🟡 backends after register: ${lib.ggml_backend_reg_count()}");
        print("🟡 devices: ${lib.ggml_backend_dev_count()}");
      } else {
        lib.llama_backend_init();
    ...
    try {
      print("🔵 calling llama_new_context_with_model");
      loadedContext = lib.llama_new_context_with_model(model, contextParams);
      print("🔵 llama_new_context_with_model returned: ${loadedContext != nullptr}");
    ...
    print("🔵 calling _initializeSampler");
    _initializeSampler(samplerParams);
    print("🔵 _initializeSampler done");

    if (mmprojPath != null && mmprojPath.isNotEmpty) {
      final mprojPathPtr = mmprojPath.toNativeUtf8().cast<Char>();
      try {
        print("🔵 mtmd_context_params_default");
        var mparam = lib.mtmd_context_params_default();
        print("🔵 mparam done, setting use_gpu");
        mparam.use_gpu = modelParamsDart.nGpuLayers != 0;
        // image_max_tokens left at default (-1) — limit via input image resize instead
        print("🔵 calling mtmd_init_from_file, use_gpu=${mparam.use_gpu}, image_max_tokens=256");
        _mctx = lib.mtmd_init_from_file(mprojPathPtr, model, mparam);
        print("🔵 mtmd_init_from_file returned: ${_mctx != nullptr}");
        if (_mctx == nullptr) {
